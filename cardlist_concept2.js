...
(function () {
  ...
  function initCardWall() {
    // FIX: `booyahSidebar` is now `position: fixed` in the CSS (was
    // `sticky`), so it needs to escape whatever Carrd wraps it in the
    // same way `sidebarBackdrop`/`booyahInspectorModal` already do —
    // otherwise a transformed Carrd ancestor can trap it in a stacking
    // context no z-index can climb out of.
    ...
    ARTISTS.forEach(function (artist, index) {
      ...
      section.innerHTML =
        '<a class="section-artist-link" href="' + artist.profileUrl + '" target="_blank" rel="noopener noreferrer">' +
