...
var ARTISTS = [
  ...
  { name: "M.McRobo",          icon: "mc.png",       profileUrl: "https://booyahtcg.com/#mmcrobo",          volumes: [{ id: "mc", label: "Volume 1", n: 10 }] },
...
function openDrawer(artist) {
  ...
  var drawerProfileBtn = document.getElementById("drawer-profile-btn");
  ...
  var count = totalCards(artist);
  var isComingSoon = artist.comingSoon || !artist.volumes || artist.volumes.length === 0 || count === 0;
  ...
  if (drawerBody) {
    ...
    if (isComingSoon) {
      var comingSoonBlock = document.createElement("div");
      ...
      var multiVolume = artist.volumes.length > 1;
      ...
      artist.volumes.forEach(function(vol) {
        ...
        for (var i = 1; i <= vol.n; i++) {
          var globalIdx = currentArtistCards.length;
          var url = cardUrl(vol, i);
          var altText = artist.name + " " + vol.label + " card " + i;
...
function updateLightboxContent() {
  ...
  var card = currentArtistCards[currentCardIndex];
  if (!card) return;
